...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        attn_collector: Optional[Callable[[torch.Tensor], None]] = None,
        ...
        if attn_collector is not None:
            with torch.no_grad():
                q_scores = q
                k_scores = k
                num_kv_heads = k_scores.size(1)
                num_q_heads = q_scores.size(1)
                if num_q_heads != num_kv_heads:
                    k_scores = k_scores.repeat_interleave(num_q_heads // num_kv_heads, dim=1, output_size=num_q_heads)
                attn_scores = torch.matmul(q_scores, k_scores.transpose(-2, -1))
                attn_scores = attn_scores / math.sqrt(q_scores.size(-1))
                if attention_bias is not None:
                    attn_scores = attn_scores + attention_bias.to(dtype=attn_scores.dtype)
                attn_probs = torch.softmax(attn_scores.float(), dim=-1)
                attn_collector(attn_probs.sum(dim=2).sum(dim=1))
